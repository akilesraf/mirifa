...
import '../Home.dart';
import '../pages/confirmaypaga/CyPRDos.dart';
import '../pages/login_page.dart';
import '../services/unservices2.dart';
import '../services/userservices.dart';

class scrRifaDos extends StatelessWidget {
  const scrRifaDos();
  @override
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Padding(
        ...
        child: Image(fit: BoxFit.cover, image: AssetImage('assets/vmw.jpeg')),
      ),
    );
  }
}
...
class _Descripcion1 extends State<Descripcion1> {
  ...
  String Vendedor2 = 'Sin conexion';
  String FechaSorteo2 = 'Sin conexion';
  String MasInfo2 = 'Sin conexion';
  ...
  void RealtimeUpdate22() {
    DatabaseReference DataRef = fire.database.ref('RifaDos').child('Vendedor');
  ...
  void RealtimeUpdate23() {
    DatabaseReference DataRef =
        fire.database.ref('RifaDos').child('DesGeneral');
  ...
  void RealtimeUpdate24() {
    DatabaseReference DataRef =
        fire.database.ref('RifaDos').child('FechaSorteo');
...
class _Numeros extends State<Numeros> {
  final databaseNum = fire.database.ref('RifaDos');
  var numSel = "";
  var numPos = "";
  var numSelKey;
  var Linkp2 = '';
  ...
  void initState() {
    super.initState();
    //items.addAll(numSel);
    _activateListeners221();
    LKP2();
  }

  void LKP2() {
    DatabaseReference DataRef = fire.database.ref('RifaDos').child('LinkP');
  ...
  void web2() async {
    ...
    await html.window.open(
        "https://www.mercadopago.com.uy/payment-link/v1/redirect?preference-id=$CodigoId=link",
        'MercadoPago');
  }

  void _activateListeners221() {
    DatabaseReference DataRef =
        fire.database.ref('RifaDos').child('Numeros').child("0");
                        ...
                        onPressed: () async {
                          ...
                          if (reg == true) {
                            if (_formulariokey.currentState.validate()) {
                              bool respuesta = await UsersServices2().saveNota(
                                _controlNombre.text,
...
class _Descripcion extends State<Descripcion> {
  ...
  String APText2 = 'Sin Conexion';
  String CMiniText2 = 'Sin Conexion';
  StreamSubscription _iniciopeticiones;
  @override
  void initState() {
    super.initState();
  ...
  void RealtimeUpdate2226() {
    DatabaseReference DataRef =
        fire.database.ref('RifaDos/Descripcion').child('CMini');
        ...
        Card(
          ...
          child: Column(
            children: <Widget>[
              ...
              Text(
                'Cantidad min. para realizar el sorteo: $CMiniText2',
                style:
                    const TextStyle(fontWeight: FontWeight.bold, fontSize: 20),
              ),
            ],
